...
export default function AdForm({ property_action, property_type }) {
  ...
  const uploadSingleFile = async (event) => {
    ...
    try {
      const CloudImg = await axios.post(
        `${UploadFile_API}/uploadSingleFiles`,
        event.target.files
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data?.error) {
        ...
        toast.success("Porperty Updated successfully");
        navigate("/dashboard");
        console.log(data);
      }
    } catch (err) {
  ...
  const handleDelete = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `${Property_API}/deleteProperty/${param._id}`,
        ad,
        {
          headers: {
            token: auth.token,
  ...
  return (
    <div className="container offset">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button disabled={ad.loading} className="btn btn-success">
          {ad.loading ? "Saving..." : "Submit"}
        </button>
      </form>

      <button onClick={handleDelete} disabled={ad.loading} className="btn btn-warning mt-3 mb-2">
        {ad.loading ? "Deleting..." : "Delete Property"}
      </button>
